...
python -m table2text.launchers.main_111321 --test_run False
"""
    ...
    **additional_kwargs,  # Feed in things like `--skip_generation yes`.
    ...
    if test_run:
        ...
        kwargs = shared.get_best_hyper_params(
            ...
            seed=0, model_name_or_path="gpt2", date="111321",
        )
        ...
        for seed in seeds:
            ...
            for target_epsilon in (0.1, 0.5, 2, 3, 5, 8):
                for target_delta in (1e-5,):
                    kwargs = shared.get_best_hyper_params(
                        tuning_mode="full", task_mode="e2e", non_private="no", target_epsilon=target_epsilon,
                        seed=seed, model_name_or_path="gpt2", date="111321", target_delta=target_delta,
                        gpu="a100",
                    )
                    command = shared.get_command(
